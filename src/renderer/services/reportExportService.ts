...
function getCoverData(p: ExcelExportParams, count: number, colCount: number): any {
  ...
  const bg = C.white
  const getBlkRow = () => Array(colCount).fill(null).map(() => mkCell('', { fill: { fgColor: { rgb: bg } } }))
  
  const pushRow = (row: XCell[], hpt?: number) => {
    rows.push(row)
  ...
  // 1. Company Name
  pushRow(getBlkRow(), 10)
  let r = pushRow(getBlkRow(), 20)
  rows[r][0] = mkCell('Sameer Ishaq Harmoudi', { font: { name: FN, bold: true, sz: 14, color: { rgb: C.black } }, fill: { fgColor: { rgb: bg } }, alignment: { horizontal: 'center', vertical: 'center' } })
  merges.push({ s: { r, c: 0 }, e: { r, c: colCount - 1 } })

  // 2. Divider line
  r = pushRow(getBlkRow(), 15)
  for (let c = 0; c < colCount; c++) {
    rows[r][c].s = { ...rows[r][c].s, border: { bottom: bdr(C.grayBorder) } }
  }

  pushRow(getBlkRow(), 15)

  // 3. Report Title
  r = pushRow(getBlkRow(), 24)
  rows[r][0] = mkCell(p.reportTitle.toUpperCase(), { font: { name: FN, bold: true, sz: 16, color: { rgb: C.black } }, fill: { fgColor: { rgb: bg } }, alignment: { horizontal: 'center', vertical: 'center' } })
  merges.push({ s: { r, c: 0 }, e: { r, c: colCount - 1 } })

  // 4. Subtitle
  r = pushRow(getBlkRow(), 18)
  const portfolio = p.module === 'Property' ? 'All Properties' : 'Investment Portfolio'
  rows[r][0] = mkCell(portfolio, { font: { name: FN, bold: false, sz: 12, color: { rgb: '64748B' } }, fill: { fgColor: { rgb: bg } }, alignment: { horizontal: 'center', vertical: 'center' } })
  merges.push({ s: { r, c: 0 }, e: { r, c: colCount - 1 } })

  pushRow(getBlkRow(), 20)
  ...
  const generatedOn = now.toLocaleString('en-GB', dateOptions).replace(',', '')

  const metaItems = [
    { label: 'Reporting Period', val: p.periodLabel },
    { label: 'Currency', val: p.currency },
    { label: 'Generated By', val: p.generatedBy },
    { label: 'Generated On', val: generatedOn },
    { label: 'Total Records', val: String(count) },
    { label: 'Portfolio', val: p.module === 'Property' ? 'Properties Management' : 'Investment Management' }
  ]

  // Calculate column splits
  const c1 = 0
  const c1End = Math.max(0, Math.floor(colCount / 3) - 1)
  const c2 = c1End + 1
  const c2End = Math.max(c2, Math.floor(colCount * 2 / 3) - 1)
  const c3 = c2End + 1
  const c3End = colCount - 1

  const addMetaRow = (i1: number, i2: number, i3: number) => {
    let rowIdx = pushRow(getBlkRow(), 18)
    const setCell = (cStart: number, cEnd: number, item: any) => {
      if (item) {
        rows[rowIdx][cStart] = mkCell(`${item.label}: ${item.val}`, { font: { name: FN, bold: true, sz: 10, color: { rgb: '64748B' } }, fill: { fgColor: { rgb: bg } }, alignment: { horizontal: 'center', vertical: 'center' } })
        if (cEnd > cStart) merges.push({ s: { r: rowIdx, c: cStart }, e: { r: rowIdx, c: cEnd } })
      }
    }
    setCell(c1, c1End, metaItems[i1])
    setCell(c2, c2End, metaItems[i2])
    setCell(c3, c3End, metaItems[i3])
  }
