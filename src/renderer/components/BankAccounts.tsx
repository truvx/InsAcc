...
export default function BankAccounts({ currency = 'AED', dateFormat = 'DD/MM/YYYY', language = 'English', bankAccounts, setBankAccounts, bankTransactions, setBankTransactions, onAuditEvent }: Props) {
  ...
  const handleDeleteTransaction = () => {
    ...
    if (deleted) {
      ...
      onAuditEvent?.(recordModuleEvent('Bank Accounts', 'Delete', account?.accountName || 'unknown', deleted.id, `Deleted ${deleted.type} transaction: ${deleted.description} ${currency}${deleted.amount.toLocaleString()}`))
    }
